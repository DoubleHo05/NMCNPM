...
const getDashboardStats = async (req, res) => {
  try {
    ...
    const todayRevenueResult = await prisma.$queryRaw`
      SELECT COALESCE(SUM(TongTien), 0) as total FROM hoadonbansach 
      WHERE DATE(NgayBan) = DATE(${todayStart})
    `;
    ...
    const monthlyRevenueFormatted = salesData.map(s => ({
      month: s.name,
      revenue: s.sales
    }));

    // Format topProducts with soldQuantity
    const topProducts = topBooksWithAuthor.map(b => ({
      id: b.id,
      title: b.title,
      author: b.author,
      price: b.price,
      imageUrl: b.imageUrl,
      soldQuantity: b.totalSold
    }));
