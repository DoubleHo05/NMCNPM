...
const PaymentModal: React.FC<PaymentModalProps> = ({
    ...
    // Save final values for success screen (since cart clears after payment)
    ...
    // For success screen, use saved values
    const displayTotal = showInvoice ? finalTotal : totalAmount;
    const displayReceived = showInvoice ? finalReceived : received;
    const displayChange = displayReceived - displayTotal;
    const displayDebt = displayChange < 0 ? Math.abs(displayChange) : 0;
    ...
    const handlePayment = () => {
        // Save final values before cart clears
                ...
                <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl transform transition-all scale-100">
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl space-y-3 mb-6 border border-slate-100">
                        ...
                        <div className="flex justify-between text-sm">
                            ...
                            <span className="font-bold text-slate-900">{formatCurrency(displayTotal)}đ</span>
                        </div>
                        <div className="flex justify-between text-sm">
                            <span className="text-slate-600">Khách đưa</span>
                            <span className="font-medium text-slate-900">{formatCurrency(displayReceived)}đ</span>
                        </div>
                        <div className="border-t border-slate-200 pt-2">
                            {displayChange >= 0 ? (
                                <div className="flex justify-between text-base">
                                    <span className="font-semibold text-slate-700">Tiền thừa</span>
                                    <span className="font-bold text-green-600">{formatCurrency(displayChange)}đ</span>
                                </div>
                            ) : (
                                <div className="flex justify-between text-base">
                                    <span className="font-semibold text-red-600">Ghi nợ</span>
                                    <span className="font-bold text-red-600">{formatCurrency(displayDebt)}đ</span>
                                </div>
                            )}
                        </div>
                    </div>
