...
import { createPaymentApi } from '../services/paymentService'; // Import payment API
...
export const StoreProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const refreshBooks = async () => {
    ...
    try {
      ...
      setBooksError(err.message || 'Lỗi kết nối');
    } finally {
  ...
  const collectMoney = async (customerId: string, amount: number) => {
    const customer = customers.find(c => c.id === customerId);
    if (!customer) return { success: false, message: 'Khách hàng không tồn tại' };

    // Check rules implicitly or backend checks it? Backend should check it.
    // However, our local check respects the setting "SuDungQuyDinhThuTien"
    ...
    try {
      ...
      if (res && (res as any).success) { // Handle different response structures if needed
        // Optimistic Update
        const newCustomers = [...customers];
        ...
        return { success: false, message: (res as any).message || 'Lỗi khi tạo phiếu thu' };
      }

    } catch (err: any) {
      console.error("Collect money error", err);
