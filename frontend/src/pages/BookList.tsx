          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 text-xs uppercase tracking-wider font-semibold">
                ...
                <th className="px-4 py-3 text-center sticky right-0 bg-slate-50">Thao tác</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {currentBooks.map((book, index) => (
                ...
                >
                  <td className="px-4 py-2.5 text-slate-500 text-sm text-center">
                    {indexOfFirstItem + index + 1}
                  </td>
                  <td className="px-4 py-2.5 min-w-[250px]">
                    <div className="flex items-start gap-3">
                      <div className="w-8 h-12 bg-slate-200 rounded overflow-hidden flex-shrink-0 border border-slate-200">
                  ...
                  <td className="px-4 py-2.5 text-slate-600 text-sm whitespace-nowrap">{book.category}</td>
                  <td className="px-4 py-2.5 text-slate-600 text-sm whitespace-nowrap">{book.author}</td>
                  <td className="px-4 py-2.5">
                    <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium ${book.stock < 50 ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'
                      }`}>
                      {book.stock}
                    </span>
                  </td>
                  <td className="px-4 py-2.5 text-slate-600 text-sm whitespace-nowrap">{book.publisher}</td>
                  <td className="px-4 py-2.5 text-slate-600 text-sm">{book.publishYear}</td>
                  <td className="px-4 py-2.5 text-slate-700 text-sm font-medium whitespace-nowrap">
                    {book.price.toLocaleString()} đ
                  </td>

                  {/* Action Buttons */}
                  <td className="px-4 py-2.5 text-center sticky right-0 bg-white group-hover:bg-slate-50 shadow-[-10px_0_10px_-10px_rgba(0,0,0,0.05)] border-l border-transparent group-hover:border-slate-100" onClick={e => e.stopPropagation()}>
        ...
        {filteredBooks.length > 0 && (
          <div className="px-4 py-3 border-t border-slate-200 flex items-center justify-between bg-slate-50">
            <span className="text-xs text-slate-500">
              {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, filteredBooks.length)} / {filteredBooks.length} sách
            </span>
            <div className="flex gap-1">
              <button
                onClick={() => setCurrentPage(1)}
                disabled={currentPage === 1}
                className="p-1.5 border border-slate-300 bg-white rounded-lg text-slate-600 hover:bg-blue-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
              ...
              <div className="flex items-center px-3 font-medium text-xs text-slate-700 bg-white border border-slate-300 rounded-lg">
