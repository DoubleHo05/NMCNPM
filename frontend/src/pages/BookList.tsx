...
const BookList: React.FC = () => {
    ...
    const handleExportExcel = () => {
        ...
        // Auto fit columns (est)
        const wscols = [
            { wch: 5 }, { wch: 10 }, { wch: 30 }, { wch: 15 }, { wch: 20 }, { wch: 10 }, { wch: 20 }, { wch: 10 }, { wch: 15 }
        ];
        ws['!cols'] = wscols;

        XLSX.writeFile(wb, `Danh_Sach_Sach_${new Date().toISOString().slice(0, 10)}.xlsx`);
    };
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <div className="flex gap-3 w-full sm:w-auto">
                        ...
                        <button
                            ...
                            className="flex items-center justify-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-lg hover:bg-slate-50 font-medium text-sm shadow-sm transition-colors active:bg-slate-100"
                        >
                            <Download size={16} />
                            <span>Xuất</span>
                        </button>
                            ...
                            {currentBooks.map((book, index) => (
                                ...
                                >
                                            ...
                                            <div className="w-10 h-14 bg-slate-200 rounded overflow-hidden flex-shrink-0 border border-slate-200">
                                                <img
                                                    src={(book.imageUrl && book.imageUrl.startsWith('['))
                                                        ...
                                                        : (book.imageUrl || 'https://placehold.co/40x60?text=No+Image')}
                                                    alt=""
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${book.stock < 50 ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'
                                            }`}>
                                            {book.stock}
                                        </span>
                                    </td>
